...
public class Shoot : MonoBehaviour
{
   ...
   void CanShootAgain()
    ...
    void Update()
    {
        ...
        parent.transform.rotation = Quaternion.Slerp(parent.transform.rotation, lookRotation, turnSpeed);

        float? angle = RotateTurrent();

        if (angle != null && Vector3.Angle(direction, parent.transform.forward) < 1)
        {
            Fire();
